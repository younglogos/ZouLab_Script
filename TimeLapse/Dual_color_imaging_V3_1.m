...
bin = 1;
basepath = 'H:\ZouLab\YJunqi\Screening method\System test\20220307 Gramicidin test\Dish2\test';
subfolder = '\';
...
%% background checking
colordef white
[~, intens_forbkg,intens2_bkg] = clicky_fullscreen(mov, bin, mean(mov(:,:,1:end),3), bkg ,200 ,name1,wvlt1);
...
%% analysis
[roi_points, intens2,intens] = clicky_fullscreen(mov2, bin, mean(mov2(:,:,1:end),3), bkg ,cmax2 ,name2, mov,wvlt2);
...
for n = 1:size(intens,2)
    ...
    box off
    axis tight
    saveas(gca,[pathname expGroup 'Dual-color plot region ' num2str(n) '.fig']);
    saveas(gca,[pathname expGroup 'Dual-color plot region ' num2str(n) '.png']);
    close(gcf)
    figure();
    ...
    legend( name1 , name2 ,'Location','northoutside','Orientation','horizontal')
    saveas(gca,[pathname expGroup '2 y Dual-color plot region ' num2str(n) '.fig']);
...
xlabel('time (s)')
ylim([0.8 max(intens2_norm(:))+0.1])
box off
% axis tight
saveas(gca,[pathname expGroup 'norm_intensity of '  num2str(name2) ' tight.fig']);
saveas(gca,[pathname expGroup 'norm_intensity of '  num2str(name2) ' tight.png']);
...
box off
saveas(gca,[pathname expGroup '\mean_norm_intensity of '  num2str(name2) ' tight.fig']);
saveas(gca,[pathname expGroup '\mean_norm_intensity of '  num2str(name2) ' tight.png']);
%% intens-2 (without bkg)
mkdir([pathname expGroup 'without background\'])
% ColorMatrix = get(gca,'colororder');
intens_wobkg = zeros(size(intens2));
...
%% intens2 (without bkg)
intens2_wobkg = zeros(size(intens2));
intens2_norm_wobkg = zeros(size(intens2));
bleach2 = [intens2_wobkg(1:6,:)./intens2_wobkg(1,:); intens2_wobkg(7:end,:)./intens2_wobkg(7,:)];
...
for n = 1:size(intens2,2)
    ...
    intens2_wobkg(:,n) = intens2(:,n)-intens2_bkg(n);
    
end
% pbleach2 = [smoothdata(intens2_wobkg(1:6,:)./mean(intens2_wobkg(1:3,:)), 1, 'movmedian',3); smoothdata(intens2_wobkg(7:end,:)./mean(intens2_wobkg(7:9,:)), 1, 'movmedian',3)];
% intens2_wobkg = intens2_wobkg./pbleach2;
intens2_norm_wobkg =  intens2_wobkg./intens2_wobkg(1,:);
...
xlabel('time (s)')
ylim([0.8 max(intens2_norm_wobkg(:))+0.1])
box off
% axis tight
...
box off
saveas(gca,[pathname expGroup 'without background\mean_norm_intensity of '  num2str(name2) ' tight.fig']);
saveas(gca,[pathname expGroup 'without background\mean_norm_intensity of '  num2str(name2) ' tight.png']);
...
box off
axis tight
saveas(gca,[pathname expGroup '1st order of difference_' num2str(name2) ' tight.fig']);
saveas(gca,[pathname expGroup '1st order of difference_' num2str(name2) ' tight.png']);
...
for n = 1:size(intens,2)
    xlswrite([pathname expGroup 'data_analysis.xlsx'],{['Cell ' num2str(n)]},name2,[char(65+n) '1']);
end
xlswrite([pathname expGroup 'data_analysis.xlsx'],{'Standard dev.'},name1,[char(66+n) '1']);
xlswrite([pathname expGroup 'data_analysis.xlsx'],{'Standard dev.'},name2,[char(66+n) '1']);

xlswrite([pathname expGroup 'data_analysis.xlsx'],intens_std,name1,[char(66+n) '2']);
xlswrite([pathname expGroup 'data_analysis.xlsx'],intens2_std,name2,[char(66+n) '2']);

xlswrite([pathname expGroup 'data_analysis.xlsx'],{'S.E.M.'},name1,[char(67+n) '1']);
xlswrite([pathname expGroup 'data_analysis.xlsx'],{'S.E.M.'},name2,[char(67+n) '1']);

xlswrite([pathname expGroup 'data_analysis.xlsx'],{['Average, n = ' num2str(size(intens,2))]},name1,[char(68+n) '1']);
xlswrite([pathname expGroup 'data_analysis.xlsx'],{['Average, n = ' num2str(size(intens,2))]},name2,[char(68+n) '1']);


xlswrite([pathname expGroup 'data_analysis.xlsx'],intens_sem,name1,[char(67+n) '2']);
xlswrite([pathname expGroup 'data_analysis.xlsx'],intens2_sem,name2,[char(67+n) '2']);

xlswrite([pathname expGroup 'data_analysis.xlsx'],mean(intens_norm_wobkg,2),name1,[char(68+n) '2']);
xlswrite([pathname expGroup 'data_analysis.xlsx'],mean(intens2_norm_wobkg,2),name2,[char(68+n) '2']);


xlswrite([pathname expGroup 'data_analysis.xlsx'],{'Time (s)'},name1,'A1');
xlswrite([pathname expGroup 'data_analysis.xlsx'],{'Time (s)'},name2,'A1');

xlswrite([pathname expGroup 'data_analysis.xlsx'],t_mov',name1,'A2');
xlswrite([pathname expGroup 'data_analysis.xlsx'],t_mov',name2,'A2');

xlswrite([pathname expGroup 'data_analysis.xlsx'],intens_norm_wobkg,name1,'B2');
xlswrite([pathname expGroup 'data_analysis.xlsx'],intens2_norm_wobkg,name2,'B2');
...
hold on;
seShoadow2_1 = area(t_mov(1:6), [(mean(intens2_norm_wobkg(1:6,:), 2) - intens2_sem(1:6)), (2 * intens2_sem(1:6))]);
set(seShoadow2_1(1),'Visible','off');
%set(seShoadow2(2),'FaceColor',[0.46, 0.68, 0.17]' ,'FaceAlpha',0.5, 'EdgeColor', 'none');
set(seShoadow2_1(2),'FaceColor',[0.85, 0.32, 0]' ,'FaceAlpha',0.4, 'EdgeColor', 'none');
hold on;
seShoadow2_2 = area(t_mov(7:end), [(mean(intens2_norm_wobkg(7:end,:), 2) - intens2_sem(7:end)), (2 * intens2_sem(7:end))]);
set(seShoadow2_2(1),'Visible','off');
%set(seShoadow1(2),'FaceColor',[0.85, 0.32, 0]' ,'FaceAlpha',0.4, 'EdgeColor', 'none');
set(seShoadow2_2(2),'FaceColor',[0.85, 0.32, 0]' ,'FaceAlpha',0.4, 'EdgeColor', 'none');
hold on;
...
plot2_1 = plot(t_mov(1:6), mean(intens2_norm_wobkg(1:6,:),2), 'Color', name2color,'LineWidth',1.5);
plot2_1 = plot(t_mov(7:end), mean(intens2_norm_wobkg(7:end,:),2), 'Color', name2color,'LineWidth',1.5);
legend([plot1_1, plot2_1], {name1, name2}, 'Location','northoutside','Orientation','horizontal');
box off;
axis tight;
xlabel('Time(s)');
ylabel('Relative intensity')
...
delta_intens1 = (intens_norm_wobkg - mean(intens_norm_wobkg(1:5, :)))./mean(intens_norm_wobkg(1:5,:));
delta_intens2 = (intens2_norm_wobkg - mean(intens2_norm_wobkg(1:5, :)))./mean(intens2_norm_wobkg(1:5));
delta_sem1 = std(delta_intens1')'./sqrt(size(delta_intens1, 2));
delta_sem2 = std(delta_intens2')'./sqrt(size(delta_intens2, 2));

figure()
seShoadow3_1 = area(t_mov(1:6), [(mean(delta_intens1(1:6,:), 2) - delta_sem1(1:6)), (2 * delta_sem1(1:6))]);
set(seShoadow3_1(1),'Visible','off');
%set(seShoadow1_2(2),'FaceColor',[0.85, 0.32, 0]' ,'FaceAlpha',0.4, 'EdgeColor', 'none');
set(seShoadow3_1(2),'FaceColor',[0, 1, 1]' ,'FaceAlpha',0.3, 'EdgeColor', 'none');
hold on;
seShoadow3_2 = area(t_mov(7:end), [(mean(delta_intens1(7:end,:), 2) - delta_sem1(7:end)), (2 * delta_sem1(7:end))]);
set(seShoadow3_2(1),'Visible','off');
%set(seShoadow2_2(2),'FaceColor',[0.46, 0.68, 0.17]' ,'FaceAlpha',0.4, 'EdgeColor', 'none');
set(seShoadow3_2(2), 'FaceColor', [0, 1, 1]' , 'FaceAlpha',0.3, 'EdgeColor', 'none');
hold on;
seShoadow4_1 = area(t_mov(1:6), [(mean(delta_intens2(1:6,:), 2) - delta_sem2(1:6)), (2 * delta_sem2(1:6))]);
set(seShoadow4_1(1),'Visible','off');
%set(seShoadow1_2(2),'FaceColor',[0.85, 0.32, 0]' ,'FaceAlpha',0.4, 'EdgeColor', 'none');
set(seShoadow4_1(2),'FaceColor',[1, 0, 1]' ,'FaceAlpha',0.2, 'EdgeColor', 'none');
hold on;
seShoadow4_2 = area(t_mov(7:end), [(mean(delta_intens2(7:end,:), 2) - delta_sem2(7:end)), (2 * delta_sem2(7:end))]);
set(seShoadow4_2(1),'Visible','off');
%set(seShoadow2_2(2),'FaceColor',[0.46, 0.68, 0.17]' ,'FaceAlpha',0.4, 'EdgeColor', 'none');
set(seShoadow4_2(2), 'FaceColor', [1, 0, 1]' , 'FaceAlpha',0.2, 'EdgeColor', 'none');
plot1_1 = plot(t_mov(1:6), mean(delta_intens1(1:6,:),2), 'Color', name1color,'LineWidth',1.5);
plot1_2 = plot(t_mov(7:end), mean(delta_intens1(7:end,:),2), 'Color', name1color,'LineWidth',1.5);
plot2_1 = plot(t_mov(1:6), mean(delta_intens2(1:6,:),2), 'Color', name2color,'LineWidth',1.5);
plot2_2 = plot(t_mov(7:end), mean(delta_intens2(7:end,:),2), 'Color', name2color,'LineWidth',1.5);
legend([plot1_1, plot2_1], {name1, name2}, 'Location','northoutside','Orientation','horizontal');
box off;
axis tight;
xlabel('Time(s)');
