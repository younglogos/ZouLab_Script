% we only analyze mito-region (expression and non-expression)
% and a merged picture is needed as a mask
% 20210326
%%
...
basepath = 'D:\data\test\20220118 gramicidin test\Dish1\test\';
mkdir([basepath 'merge']);
pathname = [basepath 'merge\'];
subfolder1 = '163725_40xWideField_bin2\';
subfolder2 = '170544_40xWideField_bin2\';
pathname1 = [basepath subfolder1];
pathname2 = [basepath subfolder2];

[file_list] = RScanDir(pathname1,  '\matlab variables.mat');
...
% constants
movname = '\movie.bin';
fileID = fopen([pathname1 'movie_info.txt']);
...
% load movie
fname1 = [pathname1 movname];
fname2 = [pathname2 movname];
[mov1, nframe1] = readBinMov(fname1, nrow, ncol);
[mov2, nframe1] = readBinMov(fname2, nrow, ncol);
mov = cat(3, mov1, mov2);
clear mov1;
clear mov2;
...
%mov = double(mov);
%toc;
interval = 1200; % wash time in second
repoint = 10; % restart point of imaging in frame
img = mean(mov, 3);
len = size(mov,3);
...
t_keyframe(repoint:end) = t_keyframe(repoint:end) + interval;
...
for p = 1:length(t_keyframe)
    ...
    %     mov(:,:,p)=double(mov(:,:,peak(p))+mov(:,:,peak(p)+1)+mov(:,:,peak(p)-1)+mov(:,:,peak(p)+2))-4*mean(mov(:,:,[peak(p)-6:peak(p)-2 peak(p)+3:peak(p)+7]),3);
    ...
    mov3(:,:,p)=double(mov(:,:,peak(p)-1)+mov(:,:,peak(p))+mov(:,:,peak(p)+1)+mov(:,:,peak(p)+2)+mov(:,:,peak(p)+3))-5*mean(mov(:,:,20:40),3);
    %     intens_light(p) = intens(peak(p)-1)+intens(peak(p))+intens(peak(p)+1)-3*mean(intens(20:40));% without background
    %     mov(:,:,p)=double(mov(:,:,peak(p)-1)+mov(:,:,peak(p))+mov(:,:,peak(p)+1))-3*mean(mov(:,:,20:40),3);
...
cmin_RIGHT = 50;
cmax_RIGHT = 1500;
[ROI_points_RIGHT,intens_RIGHT] = clicky_v2(mov3, imaging_flash_interval, 0, img2, cmin_RIGHT , cmax_RIGHT, '');
saveas(gca,[pathname 'analysis' '\Expressing Cells_RIGHT.fig']);
saveas(gca,[pathname 'analysis' '\Expressing Cells_RIGHT.png']);
cmin_LEFT = 50;
cmax_LEFT = 2000;
[ROI_points_LEFT,intens_LEFT] = clicky_v2(mov3, imaging_flash_interval, 0, img2, cmin_LEFT , cmax_LEFT, '');
...
% cmin = 100;
% cmax = 4000;
% [ROI_points_notmito,intens_region_notmito] = clicky_v2(mov, imaging_flash_interval, 0, img2, cmin , cmax, '')
% saveas(gca,[pathname 'analysis' '\Expressing Cells_not mito.fig']);
% saveas(gca,[pathname 'analysis' '\Expressing Cells_not mito.png']);
% cmin = 100;
% cmax = 4000;
% [ROI_points_neg_notmito,intens_negative_notmito] = clicky_v2(mov, imaging_flash_interval, 0, img2, cmin , cmax, '')
...
%%
[roi_points_background,background,~,region_square] = clicky_fullscreen(mov3, 0,img2, cmin_LEFT , cmax_LEFT, '', WL_LEFT,WL_LEFT);
...
set(gcf,'outerposition',get(0,'screensize'));
subplot(1,2,1)
seShoadow_LEFT1 = area(t_keyframe(1:repoint-1), [(mean(intens_LEFT_norm(1:repoint-1,:), 2) - intens_LEFT_sem(1:repoint-1, :)), (2 * intens_LEFT_sem(1:repoint-1, :))]);
hold on;
seShoadow_LEFT2 = area(t_keyframe(repoint:end), [(mean(intens_LEFT_norm(repoint:end, :), 2) - intens_LEFT_sem(repoint:end, :)), (2 * intens_LEFT_sem(repoint:end, :))]);
hold on;
set(seShoadow_LEFT1(1),'Visible','off');
set(seShoadow_LEFT2(1),'Visible','off');
% set(seShoadow2(1),'Visible','off');
%set(seShoadow2(2),'FaceColor',[0.46, 0.68, 0.17]' ,'FaceAlpha',0.5, 'EdgeColor', 'none');
set(seShoadow_LEFT1(2),'FaceColor',[0, 1, 1]' ,'FaceAlpha',0.3, 'EdgeColor', 'none');
set(seShoadow_LEFT2(2),'FaceColor',[0, 1, 1]' ,'FaceAlpha',0.3, 'EdgeColor', 'none');
plot(t_keyframe(1:repoint-1), mean(intens_LEFT_norm(1:repoint-1, :), 2), 'Color', getrgb(WL_LEFT),'LineWidth',2); hold on
plot(t_keyframe(repoint:end), mean(intens_LEFT_norm(repoint:end, :), 2), 'Color', getrgb(WL_LEFT),'LineWidth',2);
hold on
box off
% axis ([0 max(t) 0.90 1.30])
axis tight
...
subplot(1,2,2)
seShoadow_RIGHT1 = area(t_keyframe(1:repoint-1), [(mean(intens_RIGHT_norm(1:repoint-1, :), 2) - intens_LEFT_sem(1:repoint-1, :)), (2 * intens_LEFT_sem(1:repoint-1, :))]); hold on
seShoadow_RIGHT2 = area(t_keyframe(repoint:end), [(mean(intens_RIGHT_norm(repoint:end, :), 2) - intens_LEFT_sem(repoint:end, :)), (2 * intens_LEFT_sem(repoint:end, :))]);
hold on;
set(seShoadow_RIGHT1(1),'Visible','off');
set(seShoadow_RIGHT2(1),'Visible','off');
% set(seShoadow2(1),'Visible','off');
%set(seShoadow2(2),'FaceColor',[0.46, 0.68, 0.17]' ,'FaceAlpha',0.5, 'EdgeColor', 'none');
set(seShoadow_RIGHT1(2),'FaceColor',[1, 0, 1]' ,'FaceAlpha',0.2, 'EdgeColor', 'none');
set(seShoadow_RIGHT2(2),'FaceColor',[1, 0, 1]' ,'FaceAlpha',0.2, 'EdgeColor', 'none');
plot(t_keyframe(1:repoint-1), mean(intens_RIGHT_norm(1:repoint-1, :), 2), 'Color', getrgb(WL_RIGHT),'LineWidth',2); hold on
plot(t_keyframe(repoint:end), mean(intens_RIGHT_norm(repoint:end, :), 2), 'Color', getrgb(WL_RIGHT),'LineWidth',2);
box off
% axis ([0 max(t) 0.90 1.30])
axis tight
title({'Normalized intensity of ' name_RIGHT })
...
set(gcf,'outerposition',get(0,'screensize'));
subplot(1,2,1)
seShoadow_LEFT1 = area(t_keyframe(1:repoint-1), [(mean(intens_LEFT_dy(1:repoint-1, :), 2) - intens_LEFT_dysem(1:repoint-1, :)), (2 * intens_LEFT_dysem(1:repoint-1, :))]); hold on
seShoadow_LEFT2 = area(t_keyframe(repoint:end), [(mean(intens_LEFT_dy(repoint:end, :), 2) - intens_LEFT_dysem(repoint:end, :)), (2 * intens_LEFT_dysem(repoint:end, :))]);
hold on;
set(seShoadow_LEFT1(1),'Visible','off');
set(seShoadow_LEFT2(1),'Visible','off');
% set(seShoadow2(1),'Visible','off');
...
set(seShoadow_LEFT1(2),'FaceColor',[0, 1, 1]' ,'FaceAlpha',0.3, 'EdgeColor', 'none');
set(seShoadow_LEFT2(2),'FaceColor',[0, 1, 1]' ,'FaceAlpha',0.3, 'EdgeColor', 'none');
plot(t_keyframe(1:repoint-1), mean(intens_LEFT_dy(1:repoint-1, :), 2), 'Color', getrgb(WL_LEFT),'LineWidth',2);hold on
plot(t_keyframe(repoint:end), mean(intens_LEFT_dy(repoint:end, :), 2), 'Color', getrgb(WL_LEFT),'LineWidth',2);
box off
% axis ([0 max(t) 0.90 1.30])
axis tight
...
subplot(1,2,2)
seShoadow_RIGHT1 = area(t_keyframe(1:repoint-1), [(mean(intens_RIGHT_dy(1:repoint-1, :), 2) - intens_RIGHT_dysem(1:repoint-1, :)), (2 * intens_RIGHT_dysem(1:repoint-1, :))]); hold on
seShoadow_RIGHT2 = area(t_keyframe(repoint:end), [(mean(intens_RIGHT_dy(repoint:end, :), 2) - intens_RIGHT_dysem(repoint:end, :)), (2 * intens_RIGHT_dysem(repoint:end, :))]);
hold on;
set(seShoadow_RIGHT1(1),'Visible','off');
set(seShoadow_RIGHT2(1),'Visible','off');
% set(seShoadow2(1),'Visible','off');
...
set(seShoadow_RIGHT1(2),'FaceColor',[1, 0, 1]' ,'FaceAlpha',0.2, 'EdgeColor', 'none');
set(seShoadow_RIGHT2(2),'FaceColor',[1, 0, 1]' ,'FaceAlpha',0.2, 'EdgeColor', 'none');
plot(t_keyframe(1:repoint-1), mean(intens_RIGHT_dy(1:repoint-1, :), 2), 'Color', getrgb(WL_RIGHT),'LineWidth',2); hold on
plot(t_keyframe(repoint:end), mean(intens_RIGHT_dy(repoint:end, :), 2), 'Color', getrgb(WL_RIGHT),'LineWidth',2);
hold on
box off
% axis ([0 max(t) 0.90 1.30])
axis tight
...
figure();
for j = 1:size(mov3,3)
    imshow(mov3(:,1:size(mov3,2)/2,j),[cmin_LEFT cmax_LEFT], 'Border', 'tight')
    colormap(pseudocolor(WL_LEFT));
...
figure();
for j = 1:size(mov3,3)
    imshow(mov3(:,size(mov3,2)/2+1:end,j),[cmin_RIGHT cmax_RIGHT], 'Border', 'tight')
...
for num_tiff=1:size(mov3,3)
    imwrite(uint16(mov3(:,:,num_tiff)),[pathname  'analysis\movie\tiff\' num2str(name1) '.tif'],'WriteMode', 'append',  'Compression','none')
    imwrite(uint16(mov3(:,:,num_tiff)),[pathname  'analysis\movie\tiff\' num2str(name1) '_lapse\' num2str(name1)  '_' num2str(num_tiff,2) '.tif'],'WriteMode', 'append',  'Compression','none')
end
