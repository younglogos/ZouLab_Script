...
% Modified by Junqi Yang on 20211015
% Analyze movies with higher fps after stimulus
%%
...
subfolder = '';
basepath = 'X:\91 Data and analysis\YJunqi\Screening method\Stimulus pre-experiments\20210821 KCl pre-experiments\Dish4\test\172500_20xWideField_bin2\';
...
threshold_down = 100; % related to the global intens, Need to be auto
%threshold_up = 200;
...
for p = 1:length(t_keyframe)
    ...
    intens_light(p) = intens(peak(p)-1)+intens(peak(p))+intens(peak(p)+1)+intens(peak(p)+2)+intens(peak(p)+3)-(intens(peak(p)-4)+intens(peak(p)-3)+intens(peak(p)+4)+intens(peak(p)+5)-intens(peak(p)+6));% without background
    mov2(:,:,p)=double(mov(:,:,peak(p)-1)+mov(:,:,peak(p))+mov(:,:,peak(p)+1)+mov(:,:,peak(p)+2)+mov(:,:,peak(p)+3)-(mov(:,:,peak(p)-4)+mov(:,:,peak(p)-3)+mov(:,:,peak(p)+4)+mov(:,:,peak(p)+5)+mov(:,:,peak(p)+6)));
...
cmin_RIGHT = 50;
cmax_RIGHT = 2000;
[ROI_points_RIGHT,intens_RIGHT] = clicky_v1(mov2, img2, cmin_RIGHT, cmax_RIGHT, '');
saveas(gca,[pathname 'analysis' '\Expressing Cells_RIGHT.fig']);
saveas(gca,[pathname 'analysis' '\Expressing Cells_RIGHT.png']);
cmin_LEFT = 50;
cmax_LEFT = 2000;
[ROI_points_LEFT,intens_LEFT] = clicky_v1(mov2, img2, cmin_RIGHT, cmax_RIGHT, '');
...
legend('Cell 1', 'Cell 2','Cell 3', 'Cell 4','Cell 5', 'Cell 6','Cell 7', 'Cell 8','Cell 9', 'Cell 10','Cell 11', 'Cell 12','Cell 13', 'Cell 14','Cell 15', 'Cell 16','Cell 17', 'Cell 18','Cell 19','Cell 20','Cell 21','Cell 22','Cell 23','Location','best')
legend('boxoff')
...
intens_LEFT_dy = (intens_LEFT_norm ./ mean(intens_LEFT_norm(1:5, :),1) -1) * 100;
intens_RIGHT_dy = (intens_RIGHT_norm./mean(intens_RIGHT_norm(1:5, :),1) - 1) * 100;
