# -*- coding: utf-8 -*-
"""
Created on Fri Oct 15 00:25:11 2021

@author: @caichangjia created volpy-pipeline, @JunqiYang and @YiHan modified
"""
import cv2
import glob
import h5py
...
# %% convert .bin movie to .hdf5 format as volpy input
mnames = "X:\\91 Data and analysis\\HY\\Imaging\\HY_20211002Trafficking and Sensitivity of AceECL1-RFP in neuron\Dish1\\Cell1\\121652_40xWideField_300pA\\movie.bin"
ncol = 312
nrow = 208
#print(list(fnames.keys()))
movie_np = np.fromfile(mnames, dtype = "uint16")
total_nframe = int(np.shape(movie_np)[0]/ncol/nrow)
nframe = total_nframe
movie_shape = (nframe, nrow, ncol)
movie_np = np.reshape(movie_np, movie_shape)
fnames = h5py.File(mnames[:-4] + "_" + str(nframe) + "frames.hdf5", 'w')
dset = fnames.create_dataset("movie_" + str(nframe) + "frames", shape = (nframe,nrow,ncol), data = movie_np[0:nframe,:,:])
#fnames_part = dset[0:nframe_part,:,:]
#fnames.close()
del movie_np
...
def main():
    ...
    # %%  Load movie and ROIs
    fnames = mnames[:-4] + "_" + str(nframe) + "frames.hdf5"  # file path to movie file (hdf5 file)
    path_ROIs = mnames[:-4] + "_" + str(nframe) + "frames_ROIs.hdf5"  # file path to ROIs file
    file_dir = os.path.split(fnames)[0]
    
#%% dataset dependent parameters
    # dataset dependent parameters
    fr = 1/2.0658*1000                                        # sample rate of the movie
...
# %% SEGMENTATION
    ...
    #%% Manual_annotation to produce or modify ROIs
    import caiman.source_extraction.volpy.volpy_gui
