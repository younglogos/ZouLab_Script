...
% Movie loading path
subfolder = '';
basepath = 'C:\Users\ZouOptics\Desktop\20201130 WZQ\ASAP1 127K\3000\Cell2\163722_Square wave';
pathname = [basepath subfolder];
% constants
movname = '\movie.bin';
ncol = 176;         % x'
nrow = 96;          % y
camera_bias = 398;          % background due to camera bias (100 for bin 1x1)
dt_mov = 0.9452;    % exposure time in millisecond (1058Hz)
DAQname = '\movie_DAQ.txt';
dnsamp = 20;        % downsampling rate = DAQ rate/camera rate
DAQStart = 8001;    % DAQ steps start at this datapoint (included)
frmStart = 402;     % Movie steps start at this frame (included)
frmPer = 400;       % frames period
%frmCyc = 29;        % number of cycles
frmCyc = 20;
frmWindow_dn = 150;    % number of frames to calculate time constants
frmWindow_up = 150;
frmWindow = frmWindow_up;
...
% save clicky figure
saveas(gca,[pathname '\0clicky analysis.fig']);
saveas(gca,[pathname '\0clicky analysis.png']);
...
t_window=[0:0.9452:0.9452*0.5*(frmPer-1)];                   % millisecond
% falling exponential
dnStart = frmStart + frmPer*(0:frmCyc-1);       % marks the beginning
indmat = repmat(dnStart',1,frmWindow_dn) + repmat(0:frmWindow_dn-1,frmCyc,1);
...
fup_max = mean(upIntens(frmWindow-10:frmWindow));           % maximum value
fup_min = min(upIntens);                        % minimal value
fup = (upIntens-fup_min)/(fup_max-fup_min);     % normalize response to 0-100%

indmat_all = repmat(dnStart',1,frmPer) + repmat(0:frmPer-1,frmCyc,1);
indmat_dnall=indmat_all(:,1:0.5*frmPer);
indmat_upall=indmat_all(:,0.5*frmPer+1:400);
...
% falling expential
Max = [1, 0, 1, 0];     % upper bound for [a,b,c,d]
Min = [0.5, -5, 0, -5];    % lower bound for [a,b,c,d]
...
A1_up = F.a/(F.a+F.c);
A2_up = F.c/(F.a+F.c);
%% Sensitivity analysis (?F/F per 97mV)
% Set up time axis under a step voltage change (-71mV to +30mV)
...
SteplowIntens = mean(StepIntens(100:189));          % average the steady state low fluorescence
StephiIntens = mean(StepIntens(299:390));           % average the steady state high fluorescence  
...
% plot traces
subplot(3,4,9);
plot(t_window_dn, dnIntensMat');
axis tight;
% plot average and fit
subplot(3,4,10);
plot(t_window,fdnall,'.','Color','k'); hold on;    % average trace
plot(t_window_dn,fitTraceDn,'Color','b');          % fit curve
...
% save F-V figure
saveas(gca,[pathname '\0kinetic analysis.fig']);
saveas(gca,[pathname '\0kinetic analysis.png']);
...
saveas(gca,[pathname '\0step sensitivity analysis.fig']);
saveas(gca,[pathname '\0step sensitivity analysis.png']);
