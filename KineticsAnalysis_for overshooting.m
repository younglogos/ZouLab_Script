...
% Movie loading path
subfolder = '';
basepath = 'D:\data\20210722\Dish 2 pc3.1-CiVSD-Gamillus 4R_4A\Cell 3\175740_step -100 mV to 0 mV\';
...
frmWindow_dn = 40;    % number of frames to calculate time constants
frmWindow_up = 100;
...
% save clicky figure
saveas(gca,[pathname '\1clicky analysis.fig']);
saveas(gca,[pathname '\1clicky analysis.png']);
...
t_window=[0:dt_mov:dt_mov*0.5*(frmPer-1)];    
               % millisecond
% falling exponential
...
SteplowIntens = mean(StepIntens(100:190));          % average the steady state low fluorescence
StephiIntens = mean(StepIntens(300:390));           % average the steady state high fluorescence  
...
% save F-V figure
saveas(gca,[pathname '\1kinetic analysis.fig']);
saveas(gca,[pathname '\1kinetic analysis.png']);
