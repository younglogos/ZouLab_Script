...
period_param = 0.8; % this is for peak searching, now we have a selection step from the height, so we could increase the peak-finding frequency
...
plot([0:1:size(kernel_stack_elec,1)-1]'*dt_daq,mean(kernel_stack_elec,2),'Color',[0.99,0,0],'LineWidth',3);
box off
axis tight
xlabel('Time (ms)')
...
nback = 40;  % number of frames to extend kernel behind AP peak
nfront = 40; % number of frames to extend kernel in front of AP peak
% The spikefind_corr algorithm finds spikes in two iterations.  First it
% uses a simple threshold-and-max procedure (spikefind.m).  It generates a
% kernel which it convolves with the original data.  The maxima in the
% convolved data are taken as the revised spike times.  This procedure
...
kernel_mean = kernel_mean-mean(kernel_mean(1:10))+1;% moving the baseline to "1"
% figure();
% plot(kernel_mean);
% box off
% axis tight
...
for n = 1:nspike
    baseline_vec(n) = mean(kernel_stack(1:30,n));
    if dire == 1
    ...
    % for the detrending
    [xData, yData] = prepareCurveData( [], kernel_stack(1:30,n));
    ft = fittype( 'poly1' );
...
baseline_averaged = mean(kernel_mean(1:30));
if dire == 1
...
[xData, yData] = prepareCurveData( [], kernel_mean(1:30));
ft = fittype( 'poly1' );
[fitresult, gof] = fit( xData, yData, ft );
RMSE_mean=gof.rmse;
...
xlabel('frame')
ylabel('delta F')
title (['average of trails, DF/F = ' ,num2str(sensitivity_single*100,4), ' +- ' ,num2str(std(sensitivity_vec)./sqrt(n)*100,3) '%'])
...
box off
axis tight
title({['DF/F  = ',num2str(sensitivity_averaged*100,4),' % per AP (100 mV), averaged ' num2str(nspike) ' trails'];});
xlabel('Time (ms)')
