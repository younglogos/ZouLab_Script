...
clear all; clc;
% Movie loading path
basepath  = 'D:\data\test\20210714 Resting potential and sensitivity test\Dish2\cell6\165138_40xWideField_bin2';
subfolder = '';
...
DAQStart = 86001;    % DAQ steps start at this datapoint (included)
frmStart = 4302;     % Movie steps start at this frame (included)
frmPer = 4300;      % frames period
frmCyc = 5;        % number of cycles
frmWindow = 1600;    % number of frames to calculate time constants
frmWindow_up = 1600;    % number of frames to calculate time constants
frmWindow_dn = 1600;
% load bin movie
...
[intensN, pbleach] = rem_pbleach(intens, frmPer*3+1);
% figure(2)
% plot(t_mov, intensN)
% xlabel('Time')
...
SteplowIntens = mean(StepIntens(1580:1600));          % average the steady state low fluorescence
