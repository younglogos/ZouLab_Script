...
clear all; clc;
% Movie loading path
basepath = 'D:\data\test\20210714 Resting potential and sensitivity test\Dish3\cell11\172952_40xWideField_bin2';
subfolder = '';
...
ncol = 176;         % x
nrow = 96;          % y
camera_bias = 395.6;          % background due to camera bias (100 for bin 1x1)
dt_mov = 0.9452;    % exposure time in millisecond (1058Hz)
DAQname = '\movie_DAQ.txt';
dnsamp = 20;        % downsampling rate = DAQ rate/camera rate
DAQStart = 24001;    % DAQ steps start at this datapoint (included)
frmStart = 1202;     % Movie steps start at this frame (included)
frmPer = 1200;       % frames period
frmCyc = 14;        % number of cycles
frmWindow_up = 280;    % number of frames to calculate time constants
frmWindow_dn = 280;
frmWindow = 280;
...
indmat_dnall=indmat_all(:,0.5*frmPer+1:1200);
...
SteplowIntens = mean(StepIntens(1170:1190));          % average the steady state low fluorescence
StephiIntens = mean(StepIntens(570:590));           % average the steady state high fluorescence  
