% parameters
basepath = 'H:\ZouLab\YJunqi\Sensitivity\Cephied\20220509 Sensitivity comparison&deploarization titration\Dish3\cell3\180440_tri-100 to +100_(Baseline -30)';
subfolder = '';
...
plot(Vm_dnsamp, V_rem./min(V_rem)*100-100); %positive sensors
title('Normalized F-V curve');
xlabel('V_m (mV)');
ylabel('\DeltaF/F (%)');
axis tight;
...
temp_y = V_rem./min(V_rem)*100; %positive sens
% temp_y = temp_y(46*ratio:225*ratio); %-100 to 50 mV
temp_y = temp_y(50*ratio:229*ratio); %-100 to 100 mV
average_trace = mean(reshape(temp_y,60*ratio,3),2);  
average_trace = average_trace./average_trace(1);

temp_x = Vm_dnsamp;
% temp_x = temp_x(46*ratio:225*ratio); %-100 to 50 mV
temp_x = temp_x(50*ratio:229*ratio); %-100 to 100 mV
average_trace_Vm = mean(reshape(temp_x,60*ratio,3),2);
figure()
plot(average_trace_Vm(1:30*ratio+1),98*average_trace(1:30*ratio+1)-100,'color',getrgb(450),'LineWidth',2.25);hold on
